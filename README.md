...
>
> Публикация переносится на **Vercel + Neon + GPT-5.5 / reasoning low**: [отчёт развёртывания](docs/VERCEL_DEPLOYMENT_REPORT.md). Доступ к модели подтверждён через API metadata; настоящий production text/vision проверяется отдельным ограниченным acceptance. Ссылка на демо добавляется после проверки HTTPS.
