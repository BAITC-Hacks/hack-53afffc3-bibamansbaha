...
>
> Актуальная работа по развёртыванию и переходу на **GPT-5.5 / reasoning low**: [production-отчёт](docs/PRODUCTION_DEPLOYMENT_REPORT.md). Доступ к модели подтверждён через API metadata; это ещё не проверка настоящего ответа или vision. Предыдущие текстовые проверки относятся к gpt-4.1-mini.
...
Текущий опубликованный в Git ограничитель AI рассчитан на gpt-4.1-mini. Миграция на GPT-5.5 и отдельный постоянный бюджет публичного демо ещё не завершены: **одной замены имени модели недостаточно**. Без `MODEL_BUDGET_PATH` acceptance-ограничитель выключен. Не открывайте платные маршруты публично до настройки авторизации, runtime-бюджета и проверки развёртывания. Не удаляйте ledger ради сброса лимита.
...
Для кандидата `9d7e162`: **95 unit/integration и 24 browser E2E прошли**, также прошли typecheck, lint и production build. E2E запускает отдельный **dev-server** на порту 3001, использует Chrome, fixture-каталог, `.tmp/e2e.sqlite`, `.next-e2e` и отключённую внешнюю модель. Это не проверка опубликованного production URL.
...
Для Linux зависимости и сборка создаются заново: Windows `node_modules` и `.next` не переносятся. Требуется постоянный writable-диск; статический export и эфемерный serverless-диск не подходят для текущей SQLite-архитектуры. БД приложения и бюджетные ledger должны находиться вне заменяемого релиза. При резервном копировании нужно учитывать WAL и использовать согласованную SQLite-копию, а не только копировать основной файл активной БД.

Публичный URL, Linux production start, перезапуск сервиса, backup/restore и бюджет Brev должны быть подтверждены в [production-отчёте](docs/PRODUCTION_DEPLOYMENT_REPORT.md). До этого они не считаются выполненными.
