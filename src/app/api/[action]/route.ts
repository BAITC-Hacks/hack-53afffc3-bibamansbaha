...
import { matchRequestedLines } from '../../../server/requested-lines';
...
export async function POST(request:NextRequest,context:Context){try{
 ...
 if(action==='match'){const data=z.object({lines:z.array(lineSchema).max(100)}).parse(body);return reply({lines:await matchLines(s.id,data.lines)},s);}
 if(action==='lines'){
  const data=z.object({lines:z.array(lineSchema).max(100)}).parse(body);const prior=services.cart.requestedLines(s.id);
  const lines=data.lines.map(line=>{assertSafeText(JSON.stringify(line));const old=prior.find(p=>p.id===line.id);if(!old)throw new AppError('SOURCE_CONTEXT','Строка не найдена. Загрузите документ заново.',409);const changed=old.query!==line.query;const selection=line.selection??old.selection;const selectedId=selection==='excluded'||changed?undefined:line.selectedId;return{...old,...line,rawUnit:old.rawUnit,rawQuantity:old.rawQuantity,sourceText:old.sourceText,selectedId,selection,matches:changed?[]:old.matches};});
 ...
 if(action==='chat'){
  ...
  if(quantity){const line:RequestedLine={id:randomUUID(),query,quantity,rawQuantity:quantity,unit:requestedUnit??undefined,rawUnit:requestedUnit??undefined,source:'Текстовый запрос',sourceText:text.slice(0,300),matches,selection:'auto',selectedId:exact?.product.stock?exact.product.id:undefined,status:exact?.product.stock?'exact':'clarify'};services.cart.saveRequestedLines(s.id,[line]);if(line.selectedId)try{await services.cart.prepare(s.id,[{lineId:line.id,productId:line.selectedId,quantity}]);}catch(e){services.cart.addMessage(s.id,{role:'assistant',text:e instanceof AppError?e.message:'Не удалось подготовить предложение.'});}}
  return reply({state:await state(s)},s);
 }
