...
import { z } from 'zod';
import type { Cart, CartPlan, Message, Product, Proposal, ProposalLine } from '../shared/types';
...
export class CartService implements CartAdapter {
 ...
 async prepare(s:string,input:Selection[],excluded:string[]=[],expectedPreviousId?:string,operation:'add'|'replace'='add'):Promise<Proposal>{
  ...
  for(const [id,quantity] of quantities){const p=await this.catalog.get(id,true);const sourceLines=selections.filter(x=>x.productId===id);for(const line of sourceLines)if(needsUnitReview(line.requestedUnit,p.unit)&&line.unitConfirmed!==true)throw new AppError('UNIT_REVIEW','Проверьте несовпадение единиц и количество в единицах каталога. Автоматический пересчёт упаковок не выполняется.');validateQuantity(p,quantity);const requestedUnit=[...new Set(sourceLines.map(x=>x.requestedUnit?.trim()).filter(Boolean))].join(' / ');lines.push({product:p,quantity,lineTotalMinor:lineTotal(p.priceMinor!,quantity),...(requestedUnit?{requestedUnit,unitConfirmed:sourceLines.every(x=>!needsUnitReview(x.requestedUnit,p.unit)||x.unitConfirmed===true)}:{})});}
  ...
  return this.transaction(()=>{if(this.getCart(s).revision!==cart.revision)throw new AppError('CART_CHANGED','Корзина изменилась. Подготовьте предложение заново.',409);const old=this.currentProposal(s);if(expectedPreviousId&&old?.id!==expectedPreviousId)throw new AppError('STATE','Уже подготовлено новое предложение. Проверьте его.',409);if(old&&old.status==='awaiting_confirmation'){old.status='invalidated';this.saveProposal(s,old);}this.saveProposal(s,p);return p;});
 }
