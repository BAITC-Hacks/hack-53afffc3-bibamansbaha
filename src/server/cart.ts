...
const fingerprint=(p:Product)=>JSON.stringify([p.id,p.sku,p.priceMinor,p.stock,p.unit,p.minQuantity,p.packSize,p.attributes]);
...
export class CartService implements CartAdapter {
 ...
 close(){this.db.close();}
 createSession():Session {const s={id:randomBytes(32).toString('hex'),csrf:randomBytes(32).toString('hex'),createdAt:new Date().toISOString()};this.db.prepare('INSERT INTO sessions VALUES(?,?,?)').run(s.id,s.csrf,s.createdAt);return s;}
 ...
 async prepare(s:string,input:Selection[],excluded:string[]=[]):Promise<Proposal>{
  ...
  return this.transaction(()=>{const old=this.currentProposal(s);if(old&&old.status==='awaiting_confirmation'){old.status='invalidated';this.saveProposal(s,old);}this.saveProposal(s,p);return p;});
 }
 async confirm(s:string,input:{proposalId:string;version:number;hash:string;confirmed:boolean}):Promise<{proposal:Proposal;cart:Cart}>{
  ...
  if(current.some((p,i)=>fingerprint(p)!==fingerprint(proposal.lines[i].product))){proposal.status='invalidated';this.saveProposal(s,proposal);let replacement:Proposal|undefined;try{replacement=await this.prepare(s,proposal.lines.map(l=>({productId:l.product.id,quantity:l.quantity})),proposal.excluded);}catch{/* Stock may no longer support a replacement proposal. */}throw new AppError('DATA_CHANGED','Цена или остаток изменились. Проверьте обновлённое предложение и подтвердите заново.',409,replacement?{proposal:replacement}:undefined);}
