...
import { needsUnitReview, normalizeUnit } from '../shared/units';
...
export function planCart(cart:Cart,lines:ProposalLine[],operation:'add'|'replace'='add'):CartPlan{
 ...
 const repricedLines=operation==='add'?lines.flatMap(line=>{const old=cart.lines.find(item=>item.product.id===line.product.id);return old&&old.product.priceMinor!==line.product.priceMinor?[{productId:line.product.id,sku:line.product.sku,quantity:old.quantity,previousPriceMinor:old.product.priceMinor!,newPriceMinor:line.product.priceMinor!,deltaMinor:lineTotal(line.product.priceMinor!,old.quantity)-old.lineTotalMinor}]:[];}):[];
 return{operation,cartRevision:cart.revision,beforeMinor,afterMinor,deltaMinor:afterMinor-beforeMinor,addedMinor,repricingMinor:operation==='add'?afterMinor-beforeMinor-addedMinor:0,repricedLines,resultLines};
}
...
export class CartService implements CartAdapter {
 ...
 async prepare(s:string,input:Selection[],excluded:string[]=[],expectedPreviousId?:string,operation:'add'|'replace'='add'):Promise<Proposal>{
  this.own(s);assertSafeText(JSON.stringify({input,excluded}));const selections=selectionSchema.parse(input);const quantities=new Map<string,number>();const sourceIds=new Set<string>();for(const x of selections){if(x.lineId!==undefined){if(sourceIds.has(x.lineId))throw new AppError('SOURCE_CONTEXT','Строка спецификации повторяется. Подготовьте предложение заново.',409);sourceIds.add(x.lineId);}quantities.set(x.productId,quantitySum(x.quantity,quantities.get(x.productId)??0));}
 ...
 async updateCart(s:string,id:string,quantity:number,expectedRevision?:number,review?:{quantity:number;unit:string}):Promise<Cart>{
  ...
  if(this.getCart(s).revision!==before.revision)throw new AppError('CART_CHANGED','Корзина изменилась. Обновите состояние.',409);
  const unitChanged=!!product&&normalizeUnit(product.unit)!==normalizeUnit(existing.product.unit);
  const reviewed=expectedRevision!==undefined&&review?.quantity===quantity&&review?.unit===product?.unit;
  if(product&&(unitChanged||(quantity!==existing.quantity&&needsUnitReview(existing.requestedUnit,product.unit)))&&!reviewed)throw new AppError('UNIT_REVIEW','Единица товара или согласованное количество изменились. Уточните общий объём в единицах каталога и согласуйте строку заново. Корзина не изменена.',409);
  const unitConfirmed=reviewed||(!unitChanged&&quantity===existing.quantity&&existing.unitConfirmed);
  if(product&&fingerprint(product)!==fingerprint(existing.product)){const proposal=await this.prepare(s,[{productId:id,quantity,requestedUnit:existing.requestedUnit,unitConfirmed}],[],undefined,'replace');throw new AppError('DATA_CHANGED','Данные товара изменились. Проверьте предложение изменения корзины и подтвердите заново.',409,{proposal});}
  return this.transaction(()=>{const cart=this.getCart(s);if(cart.revision!==before.revision)throw new AppError('CART_CHANGED','Корзина изменилась. Обновите состояние.',409);const line=cart.lines.find(l=>l.product.id===id)!;if(product){line.product=product;line.quantity=quantity;line.unitConfirmed=unitConfirmed;line.lineTotalMinor=lineTotal(product.priceMinor!,quantity);}else cart.lines=cart.lines.filter(l=>l.product.id!==id);cart.totalMinor=totalMoney(cart.lines);cart.revision++;cart.updatedAt=new Date().toISOString();this.saveCart(s,cart);const p=this.currentProposal(s);if(p?.status==='awaiting_confirmation'){p.status='invalidated';this.saveProposal(s,p);}return cart;});
 }
}
