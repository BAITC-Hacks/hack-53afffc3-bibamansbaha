...
export class CartService implements CartAdapter {
 ...
 async confirm(s:string,input:{proposalId:string;version:number;hash:string;confirmed:boolean}):Promise<{proposal:Proposal;cart:Cart}>{
  ...
  if(current.some((p,i)=>fingerprint(p)!==fingerprint(proposal.lines[i].product))){
   ...
   let replacement:Proposal|undefined;try{replacement=await this.prepare(s,proposal.lines.map(l=>({productId:l.product.id,quantity:l.quantity})),proposal.excluded,proposal.id);}catch{/* Stock may no longer support a replacement, or a newer proposal already exists. */}throw new AppError('DATA_CHANGED','Цена или остаток изменились. Проверьте обновлённое предложение и подтвердите заново.',409,replacement?{proposal:replacement}:undefined);
  }
