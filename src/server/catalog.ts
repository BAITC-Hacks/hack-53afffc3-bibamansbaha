...
// The partner endpoint stalls with the default fetch keep-alive transport in this
// Windows runtime. Explicit connection close was verified against its real API.
function getCatalogJson(url:URL,authorization:string):Promise<unknown>{return new Promise((resolve,reject)=>{
 const req=httpsGet(url,{headers:{Authorization:authorization,Accept:'application/json','User-Agent':'EKT-Assistant/0.1',Connection:'close'},timeout:8000},response=>{
  if(response.statusCode!==200){response.resume();reject(new AppError('CATALOG_UNAVAILABLE',`Каталог недоступен (HTTP ${response.statusCode}).`,503));return;}
  const chunks:Buffer[]=[];let bytes=0;response.on('data',(chunk:Buffer)=>{bytes+=chunk.length;if(bytes>3_000_000){req.destroy(new Error('Response too large'));return;}chunks.push(chunk);});response.on('error',reject);response.on('end',()=>{try{resolve(JSON.parse(Buffer.concat(chunks).toString('utf8')));}catch{reject(new AppError('CATALOG_SCHEMA','Каталог вернул неподдерживаемый формат.',503));}});
 });const deadline=setTimeout(()=>req.destroy(new Error('Catalog timeout')),8000);req.on('close',()=>clearTimeout(deadline));req.on('timeout',()=>req.destroy(new Error('Catalog timeout')));req.on('error',reject);
});}
...
export class Catalog implements CatalogAdapter {
 ...
 private async request(path:string):Promise<unknown>{
  ...
  for(let attempt=0;attempt<2;attempt++)try{return await getCatalogJson(new URL(path,base),`Basic ${Buffer.from(`${process.env.EKT_API_USERNAME}:${process.env.EKT_API_PASSWORD}`).toString('base64')}`);}catch(error){if(attempt===1)throw error instanceof AppError?error:new AppError('CATALOG_UNAVAILABLE','Каталог не ответил. Попробуйте позже; корзина не изменена.',503);}
 ...
 async get(id:string,fresh=false){if(this.mode==='snapshot')throw new AppError('SNAPSHOT_UNAVAILABLE','Snapshot не настроен.',503);if(this.mode==='fixture'){const product=this.products.get(id);if(!product)throw new AppError('NOT_FOUND','Товар не найден.',404);return structuredClone(product);}if(!/^\d{1,15}$/.test(id))throw new AppError('NOT_FOUND','Некорректный ID товара.',404);const cached=this.products.get(id);if(!fresh&&cached&&Date.now()-Date.parse(cached.evidence[0].fetchedAt)<120_000)return cached;const raw=await this.request(`/api/products/detail?id=${encodeURIComponent(id)}`);const p=normalizeProduct(raw);if(p.id!==id)throw new AppError('CATALOG_SCHEMA','Каталог вернул другой товар.',503);this.products.set(id,p);return p;}
