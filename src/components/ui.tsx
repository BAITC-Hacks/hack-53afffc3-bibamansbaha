...
export function StatusBar({ state }: { state: AppState }) {
  ...
  return <div className="status-bar"><span className={`status-dot ${mode === 'live' ? 'live' : 'demo'}`}/><span>{mode === 'live' ? 'Живой каталог' : mode === 'snapshot' ? 'Снимок каталога' : 'Демонстрационные товары'}</span><span className="status-separator">/</span><span>{state.model.available === false ? 'Поиск без AI' : state.model.verified ? 'AI подключён' : state.model.configured ? 'AI · проверка при запросе' : 'Поиск без AI'}</span><span className="status-cart">Корзина прототипа</span></div>;
}
